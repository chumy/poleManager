...
@props([
    ...
    <div class="flex flex-row border h-8 w-28 rounded-lg border-gray-400 relative"  >
        <span onclick="dec{{$campo.$indice}}()" class="font-semibold border-r bg-amber-500 hover:bg-gray-800 hover:text-gray-400 border-gray-400 h-full w-9 flex align-middle rounded-l focus:outline-none cursor-pointer">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="m-2 " stroke-width="2" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" />
            </svg>
        </span>

        <input readonly type="text" name="{{$campo}}{{$indice}}" id="{{$campo}}{{$indice}}" value="{{ $valor ?? 0 }}" class="{{$campo}} outline-none w-10 focus:outline-none text-center bg-gray-800 font-semibold text-md hover:text-black focus:text-black  md:text-basecursor-default flex items-center text-gray-400" ></input>

        <span onclick="inc{{$campo.$indice}}()" 
...
<script>
    ...
    function inc{{$campo.$indice}} ()
    {
        let counter=document.getElementById('{{$campo}}{{$indice}}').value;
        //console.log(counter)
        counter++;
        @if ($max)
            counter=(counter > {{$max}}) ? {{$max}} : counter;
        @endif
        document.getElementById('{{$campo}}{{$indice}}').value = counter;
    };

    function dec{{$campo.$indice}} () 
    {
        let counter=document.getElementById('{{$campo}}{{$indice}}').value;
        counter--;
